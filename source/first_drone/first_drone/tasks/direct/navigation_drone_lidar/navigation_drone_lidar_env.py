...
class NavigationDroneLidarEnv(DirectRLEnv):
    def __init__(self, cfg, render_mode: str | None = None, **kwargs):
        ...
        print(f"[NavigationDroneLidarEnv] Successfully loaded JIT Policy from: {cfg.llc_checkpoint_path}")
    ...
    def _get_observations(self) -> dict:
        # High-Level Agent Observation (7 + 91 = 98 dim)
        ...
        # pos_w shape: (num_envs, 3) - sensor/drone position in world frame
        ray_hits_w = self._lidar.data.ray_hits_w
        lidar_origin = self._lidar.data.pos_w  # (num_envs, 3)
        
        # Compute distance from sensor origin to each ray hit point
        # When a ray misses, ray_hits_w is set to the max_distance point along the ray direction
        lidar_distances = torch.norm(ray_hits_w - lidar_origin.unsqueeze(1), dim=-1)  # (num_envs, num_rays)
        
        # Normalize to [0, 1] range using max_distance for cleaner NN inputs
        lidar_distances = lidar_distances / self.cfg.lidar.max_distance
        # Clamp to [0, 1] to handle any numerical edge cases
        lidar_distances = lidar_distances.clamp(0.0, 1.0)
        
        # Now the agent sees everything relative to itself! (Egocentric)
        obs = torch.cat([
            pos_err_b,
            lin_vel_b,
            self._distance_to_goal.unsqueeze(-1),
            lidar_distances,
        ], dim=-1)
        return {"policy": obs}
