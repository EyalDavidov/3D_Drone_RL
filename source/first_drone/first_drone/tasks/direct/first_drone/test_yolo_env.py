...
from .test_yolo_env_cfg import TestYoloEnvCfg


class TestYoloEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: TestYoloEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Initialize the perception module in ACTUAL model mode for the Test Env!
        self.perception = PerceptionModule(use_mock=False)
    # ------------------------------------------------------------------
    # Scene setup
    # ------------------------------------------------------------------
    def _setup_scene(self):
        ...
        # Disable the room USD so there are no walls! Just the terrain plane!
        ...
        person_usd = "http://omniverse-content-production.s3-us-west-2.amazonaws.com/Assets/Characters/Reallusion/Worker/Worker.usd"
        # Scale parameter down if the worker is gigantic (in cm vs meters) - Isaac usually uses meters. 
        # Adding a scale parameter to shrink the person down to normal size relative to the environment
        person_cfg = sim_utils.UsdFileCfg(
            usd_path=person_usd,
            scale=(0.01, 0.01, 0.01)  # Scale down to 1% if it was originally in cm
        )
        
        # Spawn ONE person 3.5 meters in front of the drone on the X axis.
        # Rotated 90 degrees left (facing sideways towards Y) -> Quaternion: W=0.7071, X=0, Y=0, Z=0.7071
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        wall_bound = 100.0 # Disabled invisible walls for YOLO orbit test
        hit_wall = (
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # --- Reset robot state ---
        # Drone starts directly looking at the person
        default_root_state = self._robot.data.default_root_state[env_ids].clone()
        # Exactly in the center X=0, Y=0
        default_root_state[:, 0] = self._terrain.env_origins[env_ids, 0]
        default_root_state[:, 1] = self._terrain.env_origins[env_ids, 1]
        # Hovering nicely at chest height of the person Z=1.0m
        default_root_state[:, 2] = 1.0
        
        # איפוס מוחלט של הסיבוב ככה שהרחפן יפנה בדיוק קדימה (ציר X) איפה שהאדם עומד
