...
class SACDroneEnvCfg(DirectRLEnvCfg):
    ...
    # * Goal: Allow agile dodges (strafing) while enforcing forward-facing. Massive penalty for crashes.
    ...
    w_time: float = -0.5          # Increased time penalty (from user edit)
    ...
    w_ang_vel: float = -0.1       # INCREASED to stop the weird spinning habit
    w_action: float = -0.01       # slightly higher for smoother flights
    w_sideslip: float = -0.2      # soft lateral penalty — allows quick dodges
    goal_radius: float = 0.40     # slightly larger so it doesn't have to spin to brake
    pillar_collision_radius: float = 0.15
    # =========================================================================
